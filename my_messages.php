...
$sql="select msg_id, msg_content, YEAR(msg_time) as release_year, DATE_FORMAT(msg_time, '%m-%d %h:%m:%s') as release_date, msg_time from message where msg_receiver=". $user_id. " order by msg_time desc";
...
if(!$result){
    ...
    $msg_num = $result->num_rows;
    for($i=0;$i<$msg_num;$i++){
        $msg_array[$i]=mysqli_fetch_assoc($result);
    }
                    ...
                    <div class="post-block archive" style="opacity: 1; display: block;">
                        <div id="posts" class="posts-collapse">
                            ...
                            <span class="archive-page-counter">
                                目前共计 <?php echo $msg_num;?> 条消息。
                            </span>
                            <?php
                            $year='0';
                            for ($i=0;$i<$msg_num;$i++){
                                $msg=$msg_array[$i];
                                $msg_year=$msg['release_year'];
                                if($year!=$msg_year){
                                    $year=$msg_year;
                                ...
                                echo "<article class=\"post post-type-normal\" itemscope=\"\" itemtype=\"http://schema.org/Article\">
                                        <header class=\"post-header\" style=\"opacity: 1; display: block; transform: translateY(0px);\">
                                            <h2 class=\"post-title\">
                                                <a class=\"post-title-link\" style='margin-left: 55px;' itemprop=\"url\">
                                                    <span itemprop=\"name\">". $msg['msg_content']. "</span>
                                                </a>
                                            </h2>
                                            <div class=\"post-meta\">
                                                <time class=\"post-time\" itemprop=\"dateCreated\" datetime=\"". $msg['release_time']. "+08:00\" content=\"". $msg['release_year']. "-". $msg['release_date']."\">".
                                    $msg['release_date'].
                                    "</time>
                                            </div>
                                        </header>
